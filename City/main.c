...
unsigned int  ch = 0, w = 0, pwm = 0x0200, test = 0xd356, temp, dir=1, mode=0,datch=330, stoptimer = 6000, flag1, add = 0,curservo = 2650, prevservo = 2650, firstirda = 0;
...
float        sl3,   /* AN0, stripe sensor -> left 3 */
             ...
             ss = 1000,    /* AN7, stripe sensor -> stop */ 
/* */             
...
if(turns == 1 && sl3 < datch) add = 300;
if(turns == 1 && sr3 < datch) add = -300;
...
void recount(void){
	if(turns)
		curservo = 2650 + (int)(750 * (turn / turns) / 9) + add;		
	else curservo = prevservo;
	if(abs(curservo - 2650) < 150) {
		PORTEbits.RE1 = 0;
		curservo *= 2;
		curservo += prevservo;
		curservo /= 3;
...
int main (void)
{
    ...
    {
		recount();
		if(rl > 1100 || rr > 1100 || fl > 1100 || fr > 1100) mode = 1;
		if(mode == 0){
			pwm = 800;
			//PORTEbits.RE1 = 0;
		}
		...
		if(mode == 2){
			pwm = 700;
			if(ss < datch && abs(curservo - 2650) < 150 && turns < 7) mode = 1;
			//PORTEbits.RE1 = 1;
		}
		if(mode == 3){
			while(stoptimer){
